...
def call_otsu_threshold(img_title="/home/pi/opencv/video0005_frame0008_R128x128.png", is_reduce_noise=False):
    ...
    otsu_threshold, image_result = cv2.threshold(
        image, 30, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU,
    )
    ...
    im_color = cv2.applyColorMap(image_result, cv2.COLORMAP_BONE)
    cv2.imshow("Otsu's thresholding result", im_color)

    cv2.imwrite("otsu.jpg", image_result)
    cv2.waitKey(0)
